...
KNOWN INVENTORY ITEMS (alias ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ full name):
{item_list}
...
3. If a message is clearly about picking up / grabbing / taking supplies FROM inventory (removing items), classify it as "supply_pickup".
4. If a message is about ADDING / buying / restocking / purchasing / bringing in / replenishing supplies, classify it as "supply_addition". Key phrases: "added", "bought", "restocked", "picked up from the store", "brought in", "put back", "added to the pile". This means the stock count should go UP, not down.
5. If a message is about needing / requesting / running low on something, classify it as "need_request".
    This also applies for items NOT in the catalog. "We need a new vacuum" should be "need_request" with the raw item name.
6. If a message is about creating / placing a purchase order or ordering more of something, classify it as "create_po". This is different from need_request (which just shows what's low)
    This applies even if the item is NOT in the catalog. "create a purchase order for red shark rocket vacuum" should be "create_po" with item_name "Red Shark Rocket Vacuum".. create_po means the user wants to actually create a purchase order for a specific item.
7. If a message is a greeting or presence check like "test, you here?", "hello", "hey bot", classify it as "greeting".
8. If a message is asking about system status like "are items in ClickUp?", "is the shopping list synced?", "what's the PO status?", "show active POs", classify it as "check_status".
9. If the item or quantity is genuinely ambiguous, classify it as "unclear" and provide a friendly clarification question.
    However, if the user clearly NEEDS or WANTS a specific item that is simply not in the catalog, do NOT classify as "unclear". Instead classify as "need_request" with the raw item name. "unclear" is ONLY for when you cannot tell what they mean at all.
10. If the message has nothing to do with inventory (chit-chat, scheduling, etc.), classify it as "not_inventory".
11. Default quantity to 1 if not specified but the context is clearly about picking up a supply.
12. "handful" or "a few" = 3. "a bunch" = 5. Use reasonable defaults.
13. Improve detection of stock counts - phrases like "we actually have like 800 white rags" or "we have about 800 of X" are stock counts, even with casual language like "like" or "about".
14. Ignore lines about non-inventory commentary like dates or signatures.
15. If someone is doing a stock count / inventory count and reporting how many of each item are currently on hand (e.g., "we have 5 scrubbing bubbles, 10 magic erasers" or "stock count: scrubbing bubbles 5, lysol 3" or "counted 8 gloves large, 12 toilet brushes"), classify it as "stock_count". Key phrases: "we have", "stock count", "counted", "on hand", "in stock", "current count", "inventory count", "physical count", "update stock", "set stock". The quantities represent the TOTAL amount currently in the office, NOT what was taken.
...
you should classify this as "supply_addition" with the item "White Cleaning Cloths" and quantity 2 (because "bought" means adding stock).
If they said "i just grabbed 2 more", classify as "supply_pickup" (because "grabbed" means removing stock).
Always infer the item from thread context when the user doesn't explicitly name it.

Respond ONLY with valid JSON matching this schema:
{
  "type": "supply_pickup" | "supply_addition" | "need_request" | "create_po" | "greeting" | "check_status" | "stock_count" | "unclear" | "not_inventory",
  "items": [                          // for supply_pickup, supply_addition, AND stock_count
    {
...
def parse_inventory_message(text: str, item_catalog: list[dict], thread_context: str = "") -> dict:
    ...
    Returns
    -------
    dict  ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ parsed result with type, items, etc.
    """
    item_list_str = "\n".join(
        f"  - \"{item['alias']}\" ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ {item['name']}"
...
def parse_po_message(text: str, active_pos: list[dict]) -> dict:
    ...
    Returns
    -------
    dict  ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ parsed result with type, po_number, tracking, etc.
    """
    po_list_str = "\n".join(
...
def parse_bot_command(text: str, item_catalog: list[dict]) -> dict:
    ...
    Returns
    -------
    dict  ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ parsed command with type, item details, etc.
    """
    item_list_str = "\n".join(
...
KNOWN INVENTORY ITEMS (alias ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ full name):
{item_list}
...
def parse_inventory_message(text: str, item_catalog: list[dict], thread_context: str = "") -> dict:
    ...
    Returns
    -------
    dict  ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ parsed result with type, items, etc.
    """
    item_list_str = "\n".join(
        f"  - \"{item['alias']}\" ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ {item['name']}"
        for item in item_catalog
    )
...
def parse_po_message(text: str, active_pos: list[dict]) -> dict:
    ...
    Returns
    -------
    dict  ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ parsed result with type, po_number, tracking, etc.
    """
    po_list_str = "\n".join(
        f"  - {po['po_number']}: {po.get('quantity', '?')}x {po['item_name']} from {po.get('vendor', '?')} (status: {po.get('status', '?')})"
        for po in active_pos
...
def parse_bot_command(text: str, item_catalog: list[dict]) -> dict:
    ...
    Returns
    -------
    dict  ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ parsed command with type, item details, etc.
    """
    item_list_str = "\n".join(
        f"  - \"{item['alias']}\" -> {item['name']}"
        for item in item_catalog
